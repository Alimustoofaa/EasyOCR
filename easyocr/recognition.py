...
def recognizer_torch(model, image, text_for_pred):
    model.eval()
    with torch.no_grad():
...
def recognizer_onnx(ort_session, image):
    preds = ort_session(image)
    preds = torch.from_numpy(preds)
    return preds

def recognizer_predict(model, converter, test_loader, batch_max_length,\
                       ignore_idx, char_group_idx, decoder = 'greedy', beamWidth= 5, device = 'cpu', engine_model='torch'):
    result = []
    # model.eval()
    # with torch.no_grad():
    for image_tensors in test_loader:
        ...
        if engine_model == 'torch':
            preds = recognizer_torch(model, image, text_for_pred)
        elif engine_model == 'onnx':
            preds = recognizer_onnx(ort_session=model,image=image)
...
def get_recognizer(recog_network, network_params, character,\
                   ...
                   device = 'cpu', quantize = True, engine_model='torch'):
    ...
    if engine_model == 'torch':
        ...
        model_pkg = importlib.import_module("easyocr.model.onnx_engine")
        model = model_pkg.ONNXEngine(model_path)
...
def get_text(character, imgH, imgW, recognizer, converter, image_list,\
             ...
             adjust_contrast=0.5, filter_ths = 0.003, workers = 1, device = 'cpu', engine_model='torch'):
    ...
    result1 = recognizer_predict(recognizer, converter, test_loader,batch_max_length,\
                                 ignore_idx, char_group_idx, decoder, beamWidth, device = device, engine_model=engine_model)
